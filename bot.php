...
switch ($data->type) {  
	...
	case 'message_new': 
		...
		if (isset($data -> object -> action)) {
			$action_type = $data -> object -> action -> type;
			$action_member_id = $data -> object -> action -> member_id;

			if ($action_type == "chat_invite_user" and $action_member_id == (-1) * $group_id) {
				sendMessage("Всем привет! Из-за ограничений ВК я не смогу писать, если вы не дадите мне права администратора!
	Чтобы узнать все возмоности используйте команды -инфо или !инфо");
				break;
			}
		}
